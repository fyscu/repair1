...
//$order_map['status']=0;
        ...
        foreach ($b as $r => $v) {
            ...
            $xxx = M('refuse');
            $refuseMap['order_id'] = $v['order_id'];
            $xx = $xxx->where($refuseMap)->find();
            if ($xx) {
                $refuseStaffMap['staff_id'] = $xx['staff_id'];
                $staffRefuseUserId = $k->where($refuseStaffMap)->getField('user_id');
                $refuseUserMap['user_id'] = $staffRefuseUserId;
                $staffRefuseInfo = $c->where($refuseUserMap)->find();
                $xxxx['staffRefuseName'] = $staffRefuseInfo['name'];
                $xxxx['refuseTime'] = $xx['time'];
                $xxxx['reason'] = $xx['reason'];
                $xxxx['isRefuse'] = true;
            } else {
                $xxxx['isRefuse'] = false;
            }
            $e[$r] = array_merge($xxxx, $h, $g, $j, $m, $v);
        }
